...
#		return
	...
	if mouse_control and event is InputEventMouseButton:
		if event.is_pressed():
			drag_enabled = true
		else:
			drag_enabled = false
	elif event is InputEventMouseMotion and drag_enabled:
...
func _input_event(event):
	if event is InputEventMouseButton and event.is_action_pressed("cam_drag"):
			drag_enabled = event.pressed
			
func _input(event):
	if event is InputEventMouseButton:
		if event.is_action_pressed("cam_drag") and not event.pressed:
			drag_enabled = false
